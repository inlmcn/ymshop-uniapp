<template>
    <view class="testing-result">
        ...
        <view class="content-img" :style="{
            backgroundImage: `url(${HOME_IMG_PATH}WechatIMG1247.jpg)`,
        ...
        <!-- 主要内容区域 -->
        <view class="content" :style="{ marginTop: statusBarHeight + 45 + 'px' }">
            <view class="content-title">本检测报告基于平台随机抽样盲检，检测结果仅与该送检产品有关。检测报告仅作为平台内部质量控制之用。</view>

            <!-- 检测项目水平滚动列表 -->
            <scroll-view class="content-scroll" scroll-x="true" show-scrollbar="false">
                <view class="content-list">
                    <view class="list-item" v-for="(item, index) in itemList" :key="item.id"
                        ...
                        <view class="item-first-row">
                            ...
                            <view class="count-container">第{{ (itemList.length - index) || 0 }}次</view>
                        </view>
            ...
            <scroll-view class="choose-us-scroll" scroll-y="true">
                <image class="choose-img" :src="HOME_IMG_PATH + 'Frame 2119904207.png'"></image>
        ...
        <!-- 查看商品按钮 -->
        <view v-if="selectItemObj && selectItemObj.cpImg" class="testing-box-bottom" @click="goProductDetail">
        ...
        <uv-overlay :show="show" @click="show = false">
            <view class="warp">
                <!-- 隐藏的Canvas用于绘制分享图片 -->
                <canvas canvas-id="shareCanvas" :style="{
                ...
                <view class="warp-top" @click.stop>
                    <image v-if="canvasImage" :src="canvasImage"
                        style="width: 579rpx; height: auto; border-radius: 28rpx;" mode="widthFix" />
                    <view v-else class="loading-tip">
                        <uv-loading-icon text="正在生成分享图片..." color="#00CBCC"></uv-loading-icon>
                    </view>
                </view>
...
const shouldShowNewTag = (item) => {
    ...
    const isCurrentMonth = now.getFullYear() === date.getFullYear() &&
        now.getMonth() === date.getMonth()

    // 条件2：最近7天
    const diffTime = Math.abs(now.getTime() - date.getTime())
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24))
    const isRecentWeek = diffDays <= 7

    // 或者你可以定义其他业务逻辑
    const isNewProduct = item.isNew || false // 假设有isNew字段
...
const goProductDetail = () => {
    if (selectItemObj.value?.productId) {
        uni.navigateTo({ url: '/pages/product/detail?id=' + selectItemObj.value.productId })
    } else {
        uni.showToast({
            title: '商品信息不存在',
            icon: 'none'
...
const getSystemInfo = () => {
    uni.getSystemInfo({
        success: (res) => {
            ...
            pixelRatio.value = res.pixelRatio || 1

            // 调整canvas尺寸以适应不同设备
            // canvasWidth.value = Math.floor(579 * pixelRatio.value)
            // canvasHeight.value = Math.floor(900 * pixelRatio.value)
...
const drawCanvas = async () => {
    ...
    ctx.drawImage(backImageInfo.path, 0, 0, canvasWidth.value, canvasHeight.value)
    ...
    const continueDrawing = async () => {
        ...
        ctx.fillText(detectText, canvasWidth.value * 0.5008635578583766, canvasWidth.value * 0.81)
        // 6. 绘制产品信息区域
        ...
        ctx.fillText(productName, canvasWidth.value * 0.07253886010362694, canvasWidth.value * 1)
        // 文本换行处理
        ...
        ctx.fillText(timeLabelText, canvasWidth.value * dynamicXPosition, canvasWidth.value * 1.115)
        ...
        ctx.setFillStyle('#00CBCC')
        ctx.setTextAlign('left')
        const timeText = formatDate(selectItemObj.value?.itemList?.[selectedIndex.value]?.jcTime) || '检测时间'
        ctx.fillText(timeText, canvasWidth.value * 0.09153713298791019, canvasWidth.value * 1.2)
        ...
        let descText = selectItemObj.value?.itemList?.[selectedIndex.value]?.title?.replace(/<[^>]*>/g, '') || '';
        if (descText.length > 10) {
            descText = descText.substring(0, 12) + '...'
        }
        ctx.fillText(descText, canvasWidth.value * 0.09153713298791019, canvasWidth.value * 1.26)
        ...
        const qrCodeImg = await generateMiniProgramQrCode({ path: `pages/testing-result/index`, name: `id=${productId.value}` });
    ...
    if (selectItemObj.value?.cpImg) {
        try {
            ...
            ctx.drawImage(productImageInfo.path, productImgX, productImgY, productImgWidth, productImgHeight)
            // await new Promise((resolve) => {
            //     ctx.draw(true,() => {
            //         resolve()
            //     })
...
const shareImage = () => {
    ...
    currentPage.onShareAppMessage = () => {
        return {
            title: `多重检测报告 - ${selectItemObj.value?.itemList?.[0]?.title || ''}`,
