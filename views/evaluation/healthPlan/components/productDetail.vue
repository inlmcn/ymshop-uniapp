<template>
    <view class="product-detail">
        ...
        <!-- 商品评价 -->
        <uv-skeleton v-if="isLoadingMain" :title="false" :rowsWidth="['100%', '100%']" :loading="true" :rows="2"
            :animate="true" :rowsHeight="[40, 120]"></uv-skeleton>
        ...
        <view v-else class="comment-section fade-in">
            ...
            <template v-else>
                ...
                <swiper v-if="detailData && detailData.reply && detailData.reply.length > 0"
                    class="comment-section-swiper" :circular="true" :autoplay="true" :interval="3000">
        ...
        <view class="official-account" v-if="productList.length > 0">
            ...
            <view class="official-account-content">
                <scroll-view scroll-y="true" class="scroll-box">
                    <view class="scroll-box-item" v-for="(item, index) in productList" :key="index"
...
const emits = defineEmits(['update:maskShows'])
...
const buyNow = async () => {
    ...
    if (props.detailData && props.detailData.productValue && props.detailData.storeInfo) {
        const onSale = props.detailData.storeInfo.onSale || 1; // 从 storeInfo 获取起售数量，默认为 1
    ...
    await getCartClear()
    console.log('购物车已清空')
    uni.navigateTo({ url: '/pages/orderList/orderPay?data=' + encodeURIComponent(JSON.stringify(params)) })
}
