 ...
 * @param shareInfo
 */
async function open(data, shareInfo) {
  order.value = data
  show.value = true
  // 生成小程序二维码
  uni.showLoading({title: '获取数据中...'})
  miniProgramQrCode.value = await generateMiniProgramQrCode({path: `pages/share/index`, name: shareInfo.query});
  await nextTick(() => {
    draw()
  })
}
 ...
 */
function close() {
  order.value = posterImagePath.value = undefined
  show.value = false
}
...
async function drawUserInfo() {
  ...
  ctx.translate(68, 32)
  // y 要设置成字体大小的一半
  ctx.fillText(mainStore.user.nickname + "邀请您帮忙付款啦~", 0, 8)
  ctx.restore()
}
...
async function drawGoodsInfo(width) {
  ...
  const src = order.value?.cartInfo?.[0]?.productInfo?.image || 'https://hnapi.booseng.com/static/icon/logo.png'
  await drawImage({x: 24, y: 72, width: imageHeight, height: imageHeight, src})
  // 商品名称
  const goodsName = '您的好友' + order.value.realName + '邀请您帮忙付款'
  // 商品价格
  const goodsPrice = order.value.payPrice
  // 商品划线价格
  const goodsOldPrice = "快来帮助我付款"
...
async function drawShareInfo(width) {
  ...
  const subText = "来自「HealthCoast商城」小程序"
  ctx.font = 'normal bold 16px sans-serif';
  ctx.fillStyle = '#000000'
  // 主要字体
...
async function save() {
  ...
  try {
    await saveImageToPhotosAlbum(posterImagePath.value, '您的好友' + order.value.realName + '邀请您帮忙付款')
    close()
  } finally {
    uni.hideLoading()
  }
