...
const startEvaluation = () => {
  ...
  uni.redirectTo({
    url: `/views/evaluation/questionnaire/index?tag=0&id=${detail.value.id}&pageUrl=${'/root/review/startup'}`,
  });
};
...
onShow(() => {
  ...
  getQuestionnaireDetail({ pageUrl: '/root/review/startup' })
    .then((res) => {
      detail.value = res;
    })
    .catch((err) => {
