...
const startEvaluation = () => {
  ...
  uni.redirectTo({
    url: `/views/evaluation/questionnaire/index?fromPage=friend&tag=2&id=${detail.value.id}&pageUrl=${'/root/review/friend'}`,
  });
};
...
onShow(() => {
  ...
  getQuestionnaireDetail({ pageUrl: '/root/review/friend' })
    .then((res) => {
      detail.value = res;
    })
    .catch((err) => {
