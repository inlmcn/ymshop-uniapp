<template>
	<!-- 
		【自定义修改说明】
		修改时间: 2025-11-13
		修改内容: 为选中的标签项顶部添加图标显示功能
		影响范围: 仅在 index == activeIndex 时显示图标，不影响其他功能
		修改位置: 
		  1. 模板部分: 添加了条件渲染的 image 标签 (第30-35行)
		  2. 样式部分: 添加了 &--top-icon 样式类 (第433-441行)
	-->
	<view
		class="uv-vtabs"
		:style="[vtabsStyle]"
	>
		...
		>
			...
			>
				<!-- 分类图标 - 显示在顶部（如果存在picUrl） -->
				<image 
					v-if="item.picUrl"
					:src="item.picUrl"
					class="uv-vtabs__bar-item--top-icon"
					mode="aspectFit"
				></image>
				<view 
				...
				></view>
				<view class="uv-vtabs__bar-item--content">
					<text 
						:class="[
