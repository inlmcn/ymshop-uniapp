  ...
  computed: {
    ...
    style() {
      const style = {
        ...
        "--zui-progress-circle-ring-size": `${this.size}rpx`,
        "--zui-progress-circle-ring-width": `${this.ringWidth}rpx`,
      }
    ...
    pointerStyle() {
      ...
      style['--zui-progress-circle-pointer-rotate'] = `translate(-${offset}rpx, -50%) rotate(${rotate}deg)`
      style['--zui-progress-circle-pointer-center'] = `${offset}rpx 50%`
      return Object.keys(style)
        .map((key) => `${key}:${style[key]}`)
        .join(";");
    },
